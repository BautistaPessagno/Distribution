...
// (No Brand Kit exists yet, so kitVersion is recorded as null until the
// brand-kit ticket lands.)
...
import { FORMAT_DIMENSIONS, SlideView } from "../render/piece-slide";

export function renderSlideMarkup(doc: PieceDoc, slideIndex: number): string {
  return renderToStaticMarkup(
    createElement(SlideView, { slide: doc.slides[slideIndex], format: doc.format })
  );
}

// A full standalone HTML document for one slide — what Chromium screenshots.
export function renderSlideHtml(doc: PieceDoc, slideIndex: number): string {
  return [
    "<!DOCTYPE html>",
    '<html><head><meta charset="utf-8"><style>html,body{margin:0;padding:0;}</style></head><body>',
    renderSlideMarkup(doc, slideIndex),
    "</body></html>",
  ].join("");
}
  ...
  return {
    ...
    response: {
      ...
      preview: {
        ...
        slides: doc.slides.map((_slide, index) => renderSlideHtml(doc, index)),
      },
    },
  };
}
...
async function renderPng(doc: PieceDoc, slideIndex: number): Promise<Buffer> {
  const { width, height } = FORMAT_DIMENSIONS[doc.format];
  const browser = await getBrowser();
  const page = await browser.newPage({ viewport: { width, height }, deviceScaleFactor: 1 });
  try {
    await page.setContent(renderSlideHtml(doc, slideIndex), { waitUntil: "load" });
...
export async function exportPiece(sessionKey: string, input: unknown): Promise<GatewayResult> {
  ...
  const kitVersion: number | null = null;
  ...
  for (let index = 0; index < doc.slides.length; index += 1) {
    ...
    const png = await renderPng(doc, index);
    fs.writeFileSync(path.join(bundleDir, name), png);
    files.push({ name, kind: "png", slide: index + 1, sourceHtmlSha256: sha256(renderSlideHtml(doc, index)) });
  }
